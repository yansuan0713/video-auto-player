...
- 将 play Promise / 临时静音和可取消导航确认分别封装成模块，保留原有导航及播放策略。
- 修复移除重插播放器时 dataset 与状态表不一致、destroy 未解绑、旧 Promise 静音新源/停用页面、导航 poll 跨轮次、frame 消息 ID 冲突。
- 为 pagehide/pageshow、动态倍速播放器、媒体错误、结尾缓冲恢复和诊断面板挂起补齐生命周期。
- 调整提示模块加载顺序并支持容器全屏内提示；新增 Node 与真实 Chromium 回归测试。
